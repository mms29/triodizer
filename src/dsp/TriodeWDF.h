...
//             VkIters=iter;
//         } 
//         while (std::abs(err_k) > epsVgVk && iter < maxVkIters);
...
//         err_k = ak - Vk;
//         int ping = 0;
//         if (ping == maxPingPongIters) return;
//         do{
...
//             } while ( std::abs(err_g) > epsVgVk && iter < maxVgIters);

//             err_g = Vg_start - Vg;
//             if (abs(err_k) <= epsVgVk && abs(err_g) <= epsVgVk)
//                 break;
...
//             if (err_k < epsVgVk && err_g < epsVgVk)
//                 break;
...
//     T getTransconductance() const noexcept {
//         T G_val  = ((G[3]  * Vgk_acc + G[2])  * Vgk_acc + G[1])  * Vgk_acc + G[0];
//         T mu_val = ((mu[3] * Vgk_acc + mu[2]) * Vgk_acc + mu[1]) * Vgk_acc + mu[0];
//         T h_val  = ((h[3]  * Vgk_acc + h[2])  * Vgk_acc + h[1])  * Vgk_acc + h[0];

//         // operating variable
//         T X = Vgk_acc + Vpk_acc;
//         if (X < eps) X = eps; 
//         float denom = mu_val + h_val;
//         if (denom < eps) denom = eps;
//         // Transconductance 
//         T gm = 1.5f * (G_val / std::pow(denom, 1.5f)) * std::sqrt(X);
...
//     {
//         T tauSeconds = 1.0f;
//         alpha_acc =  1.0f - std::exp(-1.0f / (sampleRate * tauSeconds));
//     }
//     PortGType& port_g;
//     PortKType& port_k;
//     PortPType& port_p;
